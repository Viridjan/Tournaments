...
const addS=useMemo(()=>new Set(state.players.map(p=>p.name.toLowerCase())),[state.players]);
const dbE=useMemo(()=>Object.values(state.eloDb).filter(e=>e?.name&&!addS.has(e.name.toLowerCase())&&(!filter||e.name.toLowerCase().includes(filter.toLowerCase()))&&(state.testMode?!!e.test:!e.test)).sort((a,b)=>(b.elo||0)-(a.elo||0)),[state.eloDb,filter,state.testMode,addS]);
...
<div style={{display:"flex",gap:12,flexWrap:"wrap"}}><Card style={{flex:1,minWidth:260}}><h3 style={{fontSize:15,fontWeight:500,marginBottom:12}}>Add players</h3>
<div style={{position:"relative"}}>
<input type="text" value={input} placeholder="Player name..." style={S.input} onChange={e=>setInput(e.target.value)} onFocus={()=>setAcFocused(true)} onBlur={()=>setTimeout(()=>setAcFocused(false),150)} onKeyDown={e=>{if(e.key==="Enter"&&input.trim()){if(suggestions.length===1)addPlayer(suggestions[0].name);else addPlayer(input)}}}/>
{acFocused&&suggestions.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:C.card,border:`0.5px solid ${C.border}`,borderRadius:"0 0 8px 8px",boxShadow:"0 4px 12px rgba(0,0,0,0.1)",zIndex:10,maxHeight:200,overflow:"auto"}}>
{suggestions.map(s=><div key={s.name} onMouseDown={()=>addPlayer(s.name)} style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",cursor:"pointer",fontSize:13,borderBottom:`0.5px solid ${C.bL}`}} onMouseEnter={e=>e.currentTarget.style.background=C.subtle} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
<span style={{fontWeight:500}}>{s.name}</span><span style={{fontSize:11,color:C.faint}}>{s.elo}</span></div>)}</div>}
</div>
...
{state.tournamentStarted&&!p.eliminated&&<Btn onClick={()=>{if(confirm(`Abandon ${p.name}?`))dispatch({type:"ABANDON_PLAYER",index:i})}} style={{fontSize:11,padding:"2px 5px",color:C.amber,borderColor:C.aBd,flexShrink:0}}>⚑</Btn>}
<Btn onClick={()=>dispatch({type:"REMOVE_PLAYER",index:i})} style={{fontSize:11,padding:"2px 5px",flexShrink:0}}>✕</Btn></div>)}</div>
<div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.faint,marginTop:8}}><span>{state.players.length} player{state.players.length!==1?"s":""}</span><span style={{color:pc===state.players.length?C.green:C.amber}}>{pc}/{state.players.length} paid</span></div></>}</Card>
...
<tbody>{dbE.map((e,i)=>{return <tr key={e.name} style={{background:i%2===1?"#fafafa":""}}>
<td style={{padding:"5px 6px 5px 0",fontWeight:500}}>{e.name}</td><td style={{padding:"5px 6px",textAlign:"center",color:C.muted}}>{e.elo}</td>
<td style={{padding:"3px",textAlign:"center"}}><Btn onClick={()=>dispatch({type:"ADD_PLAYER",name:e.name})} style={{fontSize:11,padding:"1px 8px"}}>+</Btn></td></tr>})}</tbody></table>
</div>
