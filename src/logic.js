...
// gE/sE: get/set ELO in the database (preserves test flag)
function gE(d,n){return d[n.toLowerCase()]?.elo??ED}
function sE(d,n,e,t){return{...d,[n.toLowerCase()]:{elo:e,name:n,test:!!t}}}
// eExp: expected win probability. eCalc: compute ELO deltas (configurable K-max)
...
// findMatch: backtracking matcher — finds valid 1v1 pairings avoiding rematches
...
function getPrev(h,a){const s=new Set(a.map(p=>p.name)),m={};h.forEach(r=>r.forEach(x=>{if(x.p2==="BYE"||x.type==="multi")return;if(!s.has(x.p1)||!s.has(x.p2))return;(m[x.p1]=m[x.p1]||new Set()).add(x.p2);(m[x.p2]=m[x.p2]||new Set()).add(x.p1)}));return m}
// getPrev: map of who played whom. getByes: bye counts per player
...
// gen1v1: generate pairings. Round-robin sorts by ELO, swiss by win rate.
const so=[...ac].sort((a,b)=>ph==="roundrobin"?gE(db,b.name)-gE(db,a.name):((b.w/(b.w+b.d+b.l||1))-(a.w/(a.w+a.d+a.l||1)))||b.score-a.score);
//   Handles odd player count (bye to lowest-bye player), first-player balancing
...
// genMulti: group players into tables of min-max size for multi-player modes
...
// gpScore: Grand Prix scoring — best N of last M sessions, drop worst
...
// rkLbl/defRanks: rank labels (Winner, 2nd...) and default payout percentages
...
// calcAlloc: assigns prizes to ranked players using:
const gList=String(p.guaranteed||"").split(",").map(s=>parseInt(s.trim())).filter(n=>n>0&&n<=ar.length);
//   1. Guaranteed prizes first (G field, comma-separated rank numbers)
gList.forEach(g=>{if(p.maxQty>0){(gbr[g-1]=gbr[g-1]||[]).push({name:p.name,value:p.value,qty:1,total:p.value});p.maxQty--}});
//   2. Avoid list applied (A field, excluded from candidate pool per rank)
const aList=String(p.avoid||"").split(",").map(s=>parseInt(s.trim())).filter(n=>n>0);
//   3. Best-fit DFS search for remaining budget (round up/down by global %)
aList.forEach(a=>{(avoidMap[a-1]=avoidMap[a-1]||new Set()).add(p.name)})});
//   4. Fairness pass: ensure higher rank always gets >= lower rank value
